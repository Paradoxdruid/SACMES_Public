...
warnings.simplefilter("ignore", Warning)  # numpy polyfit_deg warning
...
global_analysis_complete: bool = (
    False  ### If analysis has completed, begin PostAnalysis
)
##################################
### Data Extraction Parameters ###
##################################
global_file_name_pattern: str = "<H><E>_<F>Hz__<N>.csv"
...
global_columns_per_electrode: int = 1
...
global_delimiter: Delimiter = Delimiter.COMMA
        ...
        match os.name:
            ...
            case "nt":  # Windows
                try:
                    ...
                    return False  # File is locked, wait and retry
            case _:
                return False  # Unsupported OS, assume file is not complete
...
def get_time(file_index: int) -> float:
    ...
    exp_time = (
        os.path.getmtime(file_now) - os.path.getmtime(file_1)
    ) / 3600  # in hours
    return exp_time
...
def read_data(
    input_file_name: str, electrode: int
        ...
        with open(input_file_name, "r", encoding=str(global_file_encoding)) as mydata:
            ...
            for line in mydata:
                ...
                if first_item_is_float:
                    current_value: float = 1000000 * float(
                        check_split_list[column_index_for_current(electrode)].replace(
                            ",", ""
                        )
                    )
                    currents.append(current_value)
                    potential_value: float = float(
                        line.split(str(global_delimiter))[
...
def MultiGausFitNoise_LSE(length, options):
    ...
    xB = (
        L0
        + c[0] * ca.exp((-((v - mu[0]) ** 2)) * lambda_[0])
        + c[1] * ca.exp((-((v - mu[1]) ** 2)) * lambda_[1])
        + c[2] * ca.exp((-((v - mu[2]) ** 2)) * lambda_[2])
        + noise
    )
    eB = yB - xB
...
class MainWindow(tk.Tk):
    ...
    def __init__(self, master: tk.Tk, *args, **kwargs):
        global global_container, global_show_frames, global_high_low_dictionary
    ...
    def create_toolbar(self) -> None:
        ...
        editmenu.add_command(
            label="Customize File Format", command=self.extraction_adjustment_frame
        )
    ...
    def extraction_adjustment_frame(self) -> None:
        ...
        container_value = 0
        ttk.Label(container, text="Current is in column #:").grid(
            row=container_value, column=0
        )
        container_value += 1
        ...
        container_value = 0
        ttk.Label(container, text="Voltage is in column #:").grid(
            row=container_value, column=1
        )
        container_value += 1
        ...
        container_value += 1
        ttk.Label(container, text="Multipotentiostat\ncolumns per electrode:").grid(
            row=container_value, column=0, columnspan=2
        )
        container_value += 1
        ...
        ttk.Label(box, text="Delimiter between\ndata columns:").grid(
            row=box_value, column=0
        )
        box_value += 1
        ttk.Radiobutton(box, text="Space", variable=self.delimiter_value, value=1).grid(
            row=box_value, column=0, pady=5
        )
        box_value += 1
        ttk.Radiobutton(box, text="Tab", variable=self.delimiter_value, value=2).grid(
            row=box_value, column=0, pady=3
        )
        box_value += 1
        ttk.Radiobutton(box, text="Comma", variable=self.delimiter_value, value=3).grid(
            row=box_value, column=0, pady=3
        )
        row_value += 1
        ...
        box2_value += 1
        ttk.Radiobutton(
            box2, text="UTF-8", variable=self.file_encoding_value, value=1
        ).grid(row=box2_value, column=0, pady=5)
        box2_value += 1
        ttk.Radiobutton(
            box2, text="UTF-16", variable=self.file_encoding_value, value=2
        ).grid(row=box2_value, column=0, pady=5)
        box2_value = 0
        row_value += 1
        ...
        box3_value += 1
        self.file_name_pattern_entry = ttk.Entry(
            box3, width=30, justify="center", font=LARGE_FONT
        )
        ...
        box3_value += 1
        ttk.Label(box3, text="<E> for electrode # (Default 1 for Multichannel)").grid(
            row=box3_value, column=1
        )
        box3_value += 1
        ...
        row_value += 1
        ttk.Button(
            win, text="Apply", command=self.apply_customize_file_format_settings
        ).grid(row=row_value, column=0, pady=6)
        ttk.Button(win, text="Done", command=win.destroy).grid(
            row=row_value, column=1, pady=3
        )

    def apply_customize_file_format_settings(self) -> None:
        """Apply the user's settings for file format."""
        global \
            global_current_column, \
            global_base_column_index_for_currents, \
...
class InputFrame(ttk.Frame):
    ...
    def __init__(self, parent: ttk.Frame, controller: tk.Tk):
        ...
        ##############################################
        self.select_file_path: ttk.Button = ttk.Button(
            self,
            style="Off.TButton",
        ...
        self.no_selected_path = ttk.Label(
            self, text="No File Path Selected", font=MEDIUM_FONT, foreground="red"
        )
        self.path_warning_exists = False
        ttk.Label(self, text="Import File Label", font=LARGE_FONT).grid(
        ...
        # --- File Handle Input ---#
        ttk.Label(self, text="Exported File Handle:", font=LARGE_FONT).grid(
            row=row_value, column=2, columnspan=2
        )
        self.filehandle = ttk.Entry(self)
        now = datetime.datetime.now()
        self.filehandle.insert(
            tk.END,
        ...
        row_value += 2
        ttk.Label(self, text="", font=LARGE_FONT).grid(
            row=row_value, rowspan=2, column=0, columnspan=10
        )
        row_value += 1
        # ---File Limit Input---#
        ttk.Label(self, text="Number of Files:", font=LARGE_FONT).grid(
            row=row_value, column=0, columnspan=2, pady=4
        )
        ...
        ttk.Label(self, text="Analysis Interval (ms):", font=LARGE_FONT).grid(
            row=row_value, column=2, columnspan=2, pady=4
        )
        ...
        ttk.Label(self, text="Resize Interval", font=LARGE_FONT).grid(
            row=row_value, column=2, columnspan=2
        )
        ...
        self.electrode_listbox_frame.grid(
            row=row_value,
            column=0,
            columnspan=2,
            padx=10,
        ...
        self.electrode_label = ttk.Label(
            self.electrode_listbox_frame, text="Select Electrodes:", font=LARGE_FONT
        )
        ...
        self.single_electrode_file = ttk.Button(
            self.electrode_listbox_frame,
            text="Multichannel",
            style="On.TButton",
        ...
        self.electrode_settings_frame.grid(
        ...
        self.frequencies_listbox_frame.grid(
        ...
        self.frequency_label = tk.Label(
            self.frequencies_listbox_frame, text="Select Frequencies", font=LARGE_FONT
        )
        ...
        self.frequency_list = tk.Listbox(
            self.frequencies_listbox_frame,
            relief="groove",
            exportselection=0,
            width=5,
            font=LARGE_FONT,
        ...
        manipulate_frequencies_frame.grid(
            row=row_value, column=2, columnspan=2, padx=10, pady=10, sticky="nsew"
        )
        ttk.Label(
            manipulate_frequencies_frame, text="Enter Frequency(s)", font=MEDIUM_FONT
        ).grid(row=0, column=0, columnspan=4)
        self.frequency_entry: ttk.Entry = ttk.Entry(
            manipulate_frequencies_frame, width=8
        )
        self.frequency_entry.grid(row=1, column=0, columnspan=4)
        ttk.Button(
            manipulate_frequencies_frame, text="Add", command=self.add_frequency
        ).grid(row=2, column=0)
        ttk.Button(
            manipulate_frequencies_frame, text="Delete", command=self.delete_frequency
        ).grid(row=2, column=1)
        ttk.Button(
            manipulate_frequencies_frame,
            text="Clear",
        ...
        row_value += 1
        # --- Select KDM Method---#
        ttk.Label(self, text="Select KDM Method", font=LARGE_FONT).grid(
            row=row_value, column=0, columnspan=2
        )
        self.kdm_box = tk.Listbox(
            self,
            relief="groove",
            exportselection=0,
            font=LARGE_FONT,
            height=len(KDMMethod),
        ...
        ttk.Label(self, text="Select Analysis Method", font=LARGE_FONT).grid(
            row=row_value, column=2, columnspan=2
        )
        self.methods_box = tk.Listbox(
            self,
            relief="groove",
            exportselection=0,
            font=LARGE_FONT,
            height=len(AnalysisMethod),
            selectmode=tk.SINGLE,
            bd=3,
        )
        ...
        ttk.Label(self, text="Select Data to be Plotted", font=LARGE_FONT).grid(
            row=row_value, column=0, columnspan=2
        )
        self.plot_options = tk.Listbox(
            self,
            relief="groove",
            exportselection=0,
            font=LARGE_FONT,
            height=len(PlotSummaryMode),
            selectmode=tk.SINGLE,
            bd=3,
        )
        ...
        ttk.Label(self, text="Select Fitting Method", font=LARGE_FONT).grid(
            row=row_value, column=2, columnspan=2
        )
        self.peak_box = tk.Listbox(
            self,
            relief="groove",
            exportselection=0,
            font=LARGE_FONT,
            height=len(PeakMethod),
            selectmode=tk.SINGLE,
            bd=3,
        )
        ...
        ttk.Label(self, text="Select X-axis units", font=LARGE_FONT).grid(
            row=row_value, column=0, columnspan=4
        )
        self.x_axis_options = tk.Listbox(
            self,
            relief="groove",
            exportselection=0,
            font=LARGE_FONT,
            height=len(PlotTimeReportingMode),
            selectmode=tk.SINGLE,
            bd=3,
        )
        ...
        self.y_limit_parameter_label = ttk.Label(
            adjustment_frame, text="Select Y Limit Parameters", font=LARGE_FONT
        )
        self.y_limit_parameter_label.grid(row=0, column=0, columnspan=4, pady=5, padx=5)
        # --- Raw Data Minimum Parameter Adjustment ---#
        self.raw_data_min_parameter_label = ttk.Label(
            adjustment_frame, text="Raw Min. Factor", font=MEDIUM_FONT
        )
        ...
        self.raw_data_max_parameter_label = ttk.Label(
            adjustment_frame, text="Raw Max. Factor", font=MEDIUM_FONT
        )
        ...
        # --- Raw Data Minimum Parameter Adjustment ---#
        self.data_min_parameter_label = ttk.Label(
            adjustment_frame, text="Data Min. Factor", font=MEDIUM_FONT
        )
        ...
        # --- Raw Data Maximum Parameter Adjustment ---#
        self.data_max_parameter_label = ttk.Label(
            adjustment_frame, text="Data Max. Factor", font=MEDIUM_FONT
        )
        ...
        self.norm_data_min_parameter_label = ttk.Label(
            adjustment_frame, text="Norm. Min.", font=MEDIUM_FONT
        )
        ...
        self.norm_data_max_parameter_label = ttk.Label(
            adjustment_frame, text="Norm. Max.", font=MEDIUM_FONT
        )
        ...
        # --- Raw Data Minimum Parameter Adjustment ---#
        self.kdm_min_label = ttk.Label(
            adjustment_frame, text="KDM Min.", font=MEDIUM_FONT
        )
        ...
        # --- Raw Data Maximum Parameter Adjustment ---#
        self.kdm_max_label = ttk.Label(
            adjustment_frame, text="KDM Max. ", font=MEDIUM_FONT
        )
        ...
        self.export_data_flag.set(True)
        ttk.Checkbutton(
            self,
            variable=self.export_data_flag,
            onvalue=True,
            offvalue=False,
            text="Export Data",
        ...
        row_value += 1
        # --- Quit Button ---#
        ttk.Button(self, width=9, text="Quit", command=lambda: os._exit(0)).grid(
            row=row_value, column=0, columnspan=2, pady=10, padx=10
        )
        # --- Button to Initialize Data Analysis --#
        ttk.Button(
            self, width=9, text="Initialize", command=self.initialize_data_analysis
    ...
    def set_data_directory(self, parent) -> None:
        ...
        try:
            ...
            if not (dialog_outcome == () or dialog_outcome == ""):
                ...
                global_data_directory = (
                    f"{local_export_path[-3]}/{local_export_path[-2]}"
                )
    ...
    def select_xaxis_options(self, event: tk.Event) -> None:
        ...
        global_x_axis_mode = PlotTimeReportingMode(
            self.x_axis_options.curselection()[0]
        )
    ...
    def electrode_cur_select(self, event: tk.Event) -> None:
        ...
        global global_electrode_count, global_electrode_list, global_electrode_dict
        global_electrode_list = [
            int(self.electrode_count.get(idx))
    ...
    def frequency_cur_select(self, event: tk.Event) -> None:
        ...
        global_frequency_list = [
            self.frequency_list.get(idx) for idx in self.frequency_list.curselection()
        ]
    ...
    def initialize_data_analysis(self) -> None:
        ...
        """
        global global_file_handle, global_export_file_path
        #########################################################
...
class CheckPoint:
    ...
    def __init__(self, parent: ttk.Frame, controller: tk.Tk):
        ...
        ttk.Label(self.win, text="Searching for files...", font=HUGE_FONT).grid(
        ...
        for electrode in global_electrode_list:
            electrode_label = ttk.Label(
                self.win, text=f"{global_handle_variable}{electrode}", font=LARGE_FONT
            )
            ...
            match global_analysis_method:
                case AnalysisMethod.CONTINUOUS_SCAN:
                    for frequency in global_frequency_list:
                        label = ttk.Label(
                            frame, text=f"{frequency}Hz", foreground="red"
                        )
                ...
                case AnalysisMethod.FREQUENCY_MAP:
                    electrode_label = ttk.Label(
                        frame, text=f"E{electrode}", font=HUGE_FONT
                    )
                    electrode_label.grid(
                        row=row_value, column=column_value, pady=5, padx=5
                    )
        ...
        ttk.Button(self.win, text="Stop", command=self.stop).grid(
            row=row_value, column=0, columnspan=2, pady=5
        )
                        ...
                        if file_is_complete(myfile):
                            ...
                            if check_:
                                if not self.already_verified[self.electrode][frequency]:
                                    self.already_verified[self.electrode][frequency] = (
                                        True
                                    )
                                    if not self.stop_search:
    ...
    def verify_multi(self, myfile: str) -> bool:
        ...
        with open(myfile, "r", encoding=str(global_file_encoding)) as mydata:
            for line in mydata:
                ...
                print(f"check_split: {check_split}")
                try:
                    _ = float(check_split)
        ...
        if check_:
            list_val = (
                global_current_column
                + (self.electrode - 1) * global_columns_per_electrode
            )
...
class ContinuousScanManipulationFrame(ttk.Frame):
    ...
    def __init__(self, parent: ttk.Frame):
        ...
        global_file_label = ttk.Label(
            self, text="1", font=MEDIUM_FONT, style="Fun.TButton"
        )
        ...
        self.set_point_norm = ttk.Entry(
            self, textvariable=global_normalization_var, width=4
        )
        ...
        normalize_button = ttk.Button(
            self, text="Apply", command=self.real_time_normalization, width=10
        )
        ...
        if global_injection_selected:
            ...
            set_point_norm_label.grid(
                row=2, column=0, columnspan=4, pady=2, sticky="nsew"
            )
        ...
        set_injection_label = ttk.Label(
            self, text="Set Injection Range", font=MEDIUM_FONT
        )
        injection_button = ttk.Button(
            self, text="Apply", command=self.real_time_injection, width=10
        )
        ...
        if len(global_frequency_list) > 1:
            ...
            ttk.Label(
                self.frequency_frame, text="Drift Correction", font=MEDIUM_FONT
            ).grid(row=0, column=0, columnspan=3, pady=1, padx=5)
            # --- High Frequency Selection for KDM and Ratiometric Analysis ---#
            ttk.Label(
                self.frequency_frame, text="High Frequency", font=MEDIUM_FONT
            ...
            ttk.Label(
                self.frequency_frame, text="Low Frequency", font=MEDIUM_FONT
            ...
            ttk.Button(
                self.frequency_frame, text="Apply", command=self.real_time_kdm
        ...
        match global_peak_method:
            case PeakMethod.POLY:
                ...
                ttk.Label(
                    regression_frame,
                    text="Poly Fit Savitzky-Golay smoothing window (mV)",
                    font=MEDIUM_FONT,
                ...
                for elec in range(global_electrode_count):
                    for freq in range(len(global_frequency_list)):
                        ...
                        self.xstart_entry[elec][freq] = ttk.Entry(
                            parameter_frame[elec][freq], width=7
                        )
                        self.xstart_entry[elec][freq].insert(
                        ...
                        self.xend_entry[elec][freq] = ttk.Entry(
                            parameter_frame[elec][freq], width=7
                        )
                        self.xend_entry[elec][freq].insert(
                            tk.END, str(global_xend[elec][freq])
                        )
                        ...
                        for i in range(len(global_frequency_list)):
                            ...
                            ttk.Button(
                                ...
                                command=lambda frame_str=frame_str: self.show_frame(
                                    frame_str
                                ),
                ...
                self.adjust_parameter_button = ttk.Button(
                    regression_frame, text="Apply", command=self.adjust_parameters
                )
                self.adjust_parameter_button.grid(
                    row=5, column=0, columnspan=4, pady=10, padx=10
                )
            case PeakMethod.GAUSS:
                ttk.Label(
                    regression_frame, text="Multi-Gauss Method", font=MEDIUM_FONT
                ).grid(row=0, column=0, columnspan=4, pady=5, padx=5)
                ...
                for elec in range(global_electrode_count):
                    for freq in range(len(global_frequency_list)):
                        ...
                        ttk.Label(
                            parameter_frame[elec][freq],
                            text="Exp. Peak Location (V)",
                            font=MEDIUM_FONT,
                        ).grid(row=0, column=0)
                        self.gauss_peak_entry[elec][freq] = ttk.Entry(
                            parameter_frame[elec][freq], width=7
                        )
                        ...
                        self.gauss_baseline_entry[elec][freq].insert(
                            tk.END, str(global_gauss_baseline[elec][freq])
                        )
                        self.gauss_baseline_entry[elec][freq].grid(row=1, column=1)
                        # --- max peak height of the voltammogram ---#
                        ttk.Label(
                            parameter_frame[elec][freq],
                            text="Exp. Max Peak Height (uA)",
                            font=MEDIUM_FONT,
                        ).grid(row=0, column=2)
                        self.gauss_maxheight_entry[elec][freq] = ttk.Entry(
                            parameter_frame[elec][freq], width=7
                        )
        ...
        x_axis_control_frame.grid(
            row=row_value, column=0, columnspan=2, pady=3, padx=3, ipadx=3, sticky="ns"
        )
        ...
        ttk.Label(
            x_axis_control_frame, text="x-axis display range", font=MEDIUM_FONT
        ).grid(row=0, column=0, columnspan=4, pady=3, padx=5)
        ttk.Label(x_axis_control_frame, text="from:", font=MEDIUM_FONT).grid(
            row=1, column=0, columnspan=2, pady=3, padx=5
        )
        ttk.Label(x_axis_control_frame, text="to:", font=MEDIUM_FONT).grid(
            row=1, column=2, columnspan=2, pady=3, padx=5
        )
        ttk.Radiobutton(
            x_axis_control_frame,
            text="start +",
            variable=self.x_axis_control_left,
            value=XBound.START_PLUS.value,
        ).grid(row=2, column=0)
        self.entry_for_x_axis_control_left_start_plus = ttk.Entry(
            x_axis_control_frame, width=3
        )
        ...
        ttk.Radiobutton(
            ...
            text="now \u2014",
            variable=self.x_axis_control_left,
            value=XBound.NOW_MINUS.value,
        ).grid(row=3, column=0)
        self.entry_for_x_axis_control_left_now_minus = ttk.Entry(
            x_axis_control_frame, width=3
        )
        ...
        ttk.Radiobutton(
            x_axis_control_frame,
            text="start +",
            variable=self.x_axis_control_right,
            value=XBound.START_PLUS.value,
        ).grid(row=2, column=2)
        self.entry_for_x_axis_control_right_start_plus = ttk.Entry(
            x_axis_control_frame, width=3
        )
        ...
        self.entry_for_x_axis_control_right_now_minus = ttk.Entry(
            x_axis_control_frame, width=3
        )
        ...
        y_axis_control_frame.grid(
            row=row_value, column=2, columnspan=2, pady=3, padx=3, ipadx=3, sticky="ns"
        )
        ...
        ttk.Label(
            y_axis_control_frame, text="y-axis display range", font=MEDIUM_FONT
        ).grid(row=0, column=1, columnspan=2)
        ttk.Label(y_axis_control_frame, text="Norm. Ratio", font=MEDIUM_FONT).grid(
            row=1, column=0, columnspan=2, pady=3, padx=1
        )
        ttk.Label(y_axis_control_frame, text="KDM", font=MEDIUM_FONT).grid(
            row=1, column=2, columnspan=2, pady=3, padx=1
        )
        ttk.Radiobutton(
            y_axis_control_frame,
            text="auto",
            variable=self.y_axis_control_norm,
            value=YBound.AUTOMATIC.value,
        ).grid(row=2, column=0)
        ttk.Radiobutton(
            y_axis_control_frame,
            text="manual",
            variable=self.y_axis_control_norm,
            value=YBound.MANUAL.value,
        ).grid(row=3, column=0)
        ttk.Label(y_axis_control_frame, text="min", font=MEDIUM_FONT).grid(
            row=4, column=0
        )
        self.entry_for_y_axis_control_norm_manual_min = ttk.Entry(
            y_axis_control_frame, width=3
        )
        self.entry_for_y_axis_control_norm_manual_min.insert(
            tk.END, str(global_min_norm)
        )
        self.entry_for_y_axis_control_norm_manual_min.grid(row=4, column=1)
        ttk.Label(y_axis_control_frame, text="max", font=MEDIUM_FONT).grid(
            row=5, column=0
        )
        self.entry_for_y_axis_control_norm_manual_max = ttk.Entry(
            y_axis_control_frame, width=3
        )
        self.entry_for_y_axis_control_norm_manual_max.insert(
            tk.END, str(global_max_norm)
        )
        self.entry_for_y_axis_control_norm_manual_max.grid(row=5, column=1)
        ttk.Radiobutton(
            y_axis_control_frame,
            text="auto",
            variable=self.y_axis_control_kdm,
            value=YBound.AUTOMATIC.value,
        ).grid(row=2, column=2)
        ttk.Radiobutton(
            y_axis_control_frame,
        ...
        self.entry_for_y_axis_control_kdm_manual_min = ttk.Entry(
            y_axis_control_frame, width=3
        )
        ...
        self.entry_for_y_axis_control_kdm_manual_max = ttk.Entry(
            y_axis_control_frame, width=3
        )
        ...
        for _ in global_plot_values:
            ttk.Button(
                self,
                text=global_frame_list[frame_value],
                command=lambda frame_value=frame_value: self.show_plot(
                    global_plot_values[frame_value], frame_value
                ),
        ...
        # --- Start ---#
        ttk.Button(
            self, text="Start", style="Fun.TButton", command=self.skeleton_key
        ).grid(row=row_value, column=0, pady=5, padx=5)
        # --- Reset ---#
        ttk.Button(self, text="Reset", style="Fun.TButton", command=self.reset).grid(
            row=row_value, column=1, pady=5, padx=5
        )
        # row_value += 1
        # --- Quit ---#
        ttk.Button(self, text="Quit", command=lambda: os._exit(0)).grid(
            row=row_value, column=3, columnspan=4, pady=5
        )
        for row in range(row_value):
            row += 1
            self.rowconfigure(row, weight=1)
    ...
    def adjust_parameters(self) -> None:
        ...
        match global_peak_method:
            case PeakMethod.POLY:
                ...
                for elec in range(global_electrode_count):
                    for freq in range(len(global_frequency_list)):
                        global_xstart[elec][freq] = float(
                            self.xstart_entry[elec][freq].get()
                        )
                        global_xend[elec][freq] = float(
    ...
    def real_time_kdm(self) -> None:
        ...
        if (
            temp_low_frequency in global_frequency_list
        ...
        # --- if only the low_frequency does not exist ---#
            ...
            temp_low_frequency not in global_frequency_list
    ...
    def real_time_normalization(self) -> None:
        ...
        if file >= global_normalization_point:
            ...
            global_norm_warning.config(
                text=f"File {global_normalization_point}" + " has not been analyzed yet"
            )
        if global_analysis_complete:
            global_post_analysis._adjust_data()
    ...
    def apply_y_axis_settings(self) -> None:
        ...
        match YBound(self.y_axis_control_norm.get()):
            ...
            case YBound.MANUAL:
                try:
                    high_bound = float(
                        self.entry_for_y_axis_control_norm_manual_max.get()
                    )
                except ValueError:
                    high_bound = 2
                try:
                    low_bound = float(
                        self.entry_for_y_axis_control_norm_manual_min.get()
                    )
                except ValueError:
                    low_bound = 0
        ...
        match YBound(self.y_axis_control_kdm.get()):
            ...
            case YBound.MANUAL:
                try:
                    high_bound = float(
                        self.entry_for_y_axis_control_kdm_manual_max.get()
                    )
                except ValueError:
                    high_bound = 2
                try:
                    low_bound = float(
                        self.entry_for_y_axis_control_kdm_manual_min.get()
                    )
                except ValueError:
                    low_bound = 0
                if high_bound <= low_bound:
    ...
    def __init__(self, parent: ttk.Frame):
        ...
        match global_peak_method:
            case PeakMethod.POLY:
                ...
                ttk.Label(
                    regression_frame,
                    text="Real Time Analysis Manipulation",
                    font=LARGE_FONT,
                ).grid(row=0, column=0, columnspan=4, pady=5, padx=5)
                ###################################################################
                ### Real Time Manipulation of Savitzky-Golay Smoothing Function ###
                ###################################################################
                ttk.Label(
                    regression_frame, text="Savitzky-Golay Window (mV)", font=LARGE_FONT
                ...
                if self.low:
                    ...
                    # --- points discarded at the beginning of the voltammogram, xstart ---#
                    ttk.Label(
                        low_parameter_frame, text="xstart (V)", font=MEDIUM_FONT
                    ).grid(row=0, column=0)
                    ...
                    # --- points discarded at the beginning of the voltammogram, xend ---#
                    ttk.Label(
                        low_parameter_frame, text="xend (V)", font=MEDIUM_FONT
                    ).grid(row=0, column=1)
                ...
                if self.high:
                    ...
                    # --- points discarded at the beginning of the voltammogram, xstart ---#
                    ttk.Label(
                        high_parameter_frame, text="xstart (V)", font=MEDIUM_FONT
                    ).grid(row=0, column=0)
                    ...
                    # --- points discarded at the beginning of the voltammogram, xend ---#
                    ttk.Label(
                        high_parameter_frame, text="xend (V)", font=MEDIUM_FONT
                    ).grid(row=0, column=1)
                ...
                if self.high and self.low:
                    self.select_low_parameters = ttk.Button(
                        regression_frame,
                        style="Off.TButton",
                        text="f <= 50Hz",
                    ...
                    self.select_high_parameters = ttk.Button(
                        regression_frame,
                        style="On.TButton",
                        text="f > 50Hz",
            ...
            case PeakMethod.GAUSS:
                # --- Title ---#
                ttk.Label(
                    regression_frame, text="Expected Peak Location (V)", font=LARGE_FONT
                ).grid(row=1, column=0, columnspan=4, pady=1)
        ...
        for _ in global_plot_values:
            ttk.Button(
                ...
                text=global_frame_list[frame_value],
                command=lambda frame_value=frame_value: self.show_plot(
                    global_plot_values[frame_value]
                ),
            ).grid(row=row_value, column=column_value, pady=2, padx=5)
            ## allows .grid() to alternate between
            ## packing into column 1 and column 2
        ...
        row_value += 1
        # --- Quit ---#
        ttk.Button(self, text="Quit Program", command=lambda: os._exit(0)).grid(
            row=row_value, column=0, columnspan=4, pady=5
        )
        for row in range(row_value):
            row += 1
            self.rowconfigure(row, weight=1)
    ...
    def skeleton_key(self) -> None:
        ...
        if global_analysis_already_initiated:
            ...
            for figure in global_figures:
                ...
                global_animations.append(
                    ElectrochemicalAnimation(
                        fig, electrode, resize_interval=0, fargs=None
                    )
                )
                fig_count += 1
            global_analysis_already_initiated = True
...
class ContinuousScanVisualizationFrame(ttk.Frame):
    ...
    def __init__(self, electrode_frame: str, count: int, parent: ttk.Frame):
        ...
        ttk.Label(self, text=electrode_frame, font=HUGE_FONT).grid(
            row=0, column=0, pady=5, sticky="n"
        )
        ttk.Label(self, text="", font=MEDIUM_FONT).grid(
            row=0, column=1, pady=3, sticky="ne"
        )
...
class InitializeContinuousCanvas:
    ...
    def __init__(self):
        ...
        for num in range(global_electrode_count):
            ...
            global_offset_normalized_data_list[num] = [
                0.0
            ] * global_number_of_files_to_process
        ...
        for electrode_frame in global_frame_list:
            ...
            global_frame_reference = ContinuousScanVisualizationFrame(
                electrode_frame, frame_count, global_plot_container
            )
            ...
            for freq in range(length):
                ...
                match global_x_axis_mode:
                    case PlotTimeReportingMode.EXPERIMENT_TIME:
                        axs[1][freq].set_xlim(
                            0,
                            xlim_factor * global_sample_rate / 3600
        ...
        try:
            ...
            if file_is_complete(myfile):
                self.run_initialization(
                    myfile, axs, subplot_count, electrode, frequency
                )
        except FileNotFoundError:
            print(f"could not find file for electrode {electrode}")
            # --- If search time has not met the search limit keep searching ---#
            root.after(
                1000, self.initialize_subplots, axs, frequency, electrode, subplot_count
            )
    ...
    def run_initialization(
        ...
        try:
            ...
            global_xstart[global_electrode_dict[electrode]][
                global_frequency_dict[frequency]
            ] = max(potentials)
            global_xend[global_electrode_dict[electrode]][
                global_frequency_dict[frequency]
            ] = min(potentials)
            xstart = global_xstart[global_electrode_dict[electrode]][
                global_frequency_dict[frequency]
            ]
            xend = global_xend[global_electrode_dict[electrode]][
                global_frequency_dict[frequency]
            ]
            ...
            global_gauss_solver.append(
                MultiGausFitNoise_LSE(length, global_options_Gauss)
            )
            #########################################
            ...
            polynomial_coeffs = np.polyfit(
                adjusted_potentials, adjusted_currents, POLYFIT_DEGREE
            )
            ...
            # - Voltammogram -#
            axs[0][subplot_count].set_ylim(
                minimum_current - abs(global_min_raw * minimum_current),
                maximum_current + abs(global_max_raw * maximum_current),
            )
            # - PHE/AUC Data -#
            axs[1][subplot_count].set_ylim(
                data - abs(global_min_data * data), data + abs(global_max_data * data)
            )
            # - Normalized Data -#
...
class InitializeFrequencyMapCanvas:
    ...
    def __init__(self):
        ...
        for electrode_frame in global_frame_list:
            # --- create an instance of the frame and append it to the global frame dictionary ---#
            # PlotContainer is the 'parent' frame
            global_frame_reference = FrequencyMapVisualizationFrame(
                electrode_frame, frame_count, global_plot_container
            )
            # sticky must be 'nsew' so it expands and contracts with resize
            global_frame_reference.grid(row=0, column=0, sticky="nsew")
    ...
    def initialize_subplots(
        self, axs: List[List[matplotlib.axes.Axes]], electrode: int
    ) -> None:
        """Initialize the y limits of each figure depending on the y values of the first file."""
        ...
        try:
            ...
            match global_plot_summary_mode:
                ...
                case PlotSummaryMode.AUC:
                    ...
                    auc = 0
                    auc_potentials = [
                        abs(potential) for potential in adjusted_potentials
                    ]
                    auc_min = min(adjusted_currents)
                    auc_currents = [y - auc_min for y in adjusted_currents]
                    while auc_index <= len(auc_currents) - 1:
            ...
            axes[1][0].set_ylim(
                charge - abs(global_min_data * charge),
                charge + abs(global_max_data * charge),
            )
            axes[1][0].set_xlim(
                int(global_frequency_list[0]), int(global_frequency_list[-1])
            )
        except Exception as exception:
...
class ElectrochemicalAnimation:
    ...
    def _setup_blit(self) -> None:
        ...
        self._blit_cache: Dict[matplotlib.axes.Axes, Any] = {}
    ...
    def _blit_clear(
        ...
        bg_cache: Dict[matplotlib.axes.Axes, Any],
    ) -> None:
            ...
            match global_x_axis_mode:
                case PlotTimeReportingMode.EXPERIMENT_TIME:
                    axs[1][count].set_xlim(
                        0,
                        self.resize_limit * global_sample_rate / 3600
                        + global_sample_rate / 7200,
                    )
                    axs[2][count].set_xlim(
                        0,
                        self.resize_limit * global_sample_rate / 3600
                        + global_sample_rate / 7200,
                    )
                    ...
                    if len(global_frequency_list) > 1:
                        try:
                            ...
                            self._draw_next_frame(
                                framedata, fargs="ratiometric_analysis"
                            )
                        except StopIteration:
                            return None
    ...
    def _raw_generator(
        ...
        adjusted_currents: List[float]
        potentials, currents, potential_to_current_map = read_data(
            myfile, self.electrode
        )
        cut_value = 0
        for value in potentials:
            if value == 0:
        ...
        match global_peak_method:
            case PeakMethod.POLY:
                ...
                try:
                    smooth_currents = list(
                        savgol_filter(currents, sg_range, global_savitzky_golay_degree)
                    )
                    potential_to_current_map = dict(zip(potentials, smooth_currents))
                except ValueError:
                    smooth_currents = list(
                        savgol_filter(currents, 15, global_savitzky_golay_degree)
                    )
                    potential_to_current_map = dict(zip(potentials, smooth_currents))
                #######################################
                ### adjust the smooth currents to   ###
                ...
                #############################
                eval_regress: List[float] = np.polyval(
                    polynomial_coeffs, adjusted_potentials
                ).tolist()
                center_regress: List[float] = (
                    eval_regress  # Copy of polynomial fit because reserved for Gauss Method
                )
                ###############################################
            ...
            case PeakMethod.GAUSS:
                ...
                ubx = np.concatenate(
                    (
                        ...
                        [
                            var0[3] + 1 * np.sqrt(1 / (2 * var0[6])),
                            np.min(data_x),
                            np.max(data_x) + 1 * np.sqrt(1 / (2 * var0[8])),
                        ],  # means
                ...
                lbx = np.concatenate(
                    (
                        ...
                        [
                            var0[3] - 1 * np.sqrt(1 / (2 * var0[6])),
                            np.min(data_x) - 1 * np.sqrt(1 / (2 * var0[7])),
                            np.max(data_x),
                        ],  # means
                ...
                # Solve the problem
                solution = solver(
                    x0=var0, p=paramVal, ubx=ubx, lbx=lbx, ubg=ubg, lbg=lbg
                )
                theta = solution["x"].full().flatten()
                # Calculate fit
                fit = (
                    theta[9]
                    + theta[0] * np.exp(-((data_x - theta[3]) ** 2) * theta[6])
                    + theta[1] * np.exp(-((data_x - theta[4]) ** 2) * theta[7])
                    + theta[2] * np.exp(-((data_x - theta[5]) ** 2) * theta[8])
                )
                fit_center = theta[9] + theta[0] * np.exp(
                    -((data_x - theta[3]) ** 2) * theta[6]
                )
        ...
        match global_analysis_method:
            case AnalysisMethod.CONTINUOUS_SCAN:
                global_data_normalization.normalize(
        ...
        *args,
        ...
        if global_key > 0:
            while True:
                ...
                if (
                    global_frequency_list[self.count]
                    == global_high_low_dictionary[HighLow.LOW]
                ):
                    local_normalized_data_list = global_offset_normalized_data_list[
                        self.num
                    ][: len(self.file_list)]
                else:
                    local_normalized_data_list = global_normalized_data_list[self.num][
                        self.count
                    ][: len(self.file_list)]
                ####################################################
                ...
                ##########################################################
                    ...
                    if self.file >= global_injection_point:
                        ...
                        peak_injection.set_data(
                            x_axis[injection_index:], data[injection_index:]
                        )
                        # Norm Data up until injection point:
                        normalization.set_data(
                            x_axis[:injection_index],
    ...
    def _ratiometric_animation(
        ...
        local_norm = [
            x * 100 for x in global_normalized_ratiometric_data_list[self.num]
        ]
        ...
        ############################################
            ...
            if self.file >= global_injection_point:
                ...
                plots[0].set_data(
                    x_axis[:injection_index], local_norm[:injection_index]
                )
                plots[1].set_data(
                    x_axis[injection_index:], local_norm[injection_index:]
                )
...
class DataNormalization:
    ...
    def normalize(
        self, file: int, data: float, num: int, count: int, index: int
    ) -> None:
        ...
        if frequency == global_high_low_dictionary[HighLow.LOW]:
            match global_x_axis_mode:
                case PlotTimeReportingMode.EXPERIMENT_TIME:
                    offset = (
                        sample * global_low_frequency_slope
                        + global_low_frequency_offset
                    )
                case PlotTimeReportingMode.FILE_NUMBER:
                    offset = (
                        file * global_low_frequency_slope + global_low_frequency_offset
                    )
        else:
            offset = 0
        ...
        # normalize to the first file ---#
            ...
            global_normalized_data_list[num][count][index] = (
                data / global_data_list[num][count][0]
            )
            ###########################################################################
            ### If this is a low frequency, apply the offset to the normalized data ###
            ###########################################################################
    ...
    def renormalize_data(self, file: int) -> None:
        ...
        if file == global_normalization_point:
                    ...
                    if (
                        ...
                        for index in range(len(global_file_list)):
                            ...
                            match global_x_axis_mode:
                                ...
                                case PlotTimeReportingMode.FILE_NUMBER:
                                    offset = (
                                        file * global_low_frequency_slope
                                        + global_low_frequency_offset
                                    )
                            global_offset_normalized_data_list[num][index] = (
                                global_normalized_data_list[num][count][index] + offset
            ...
            global_norm_warning.config(
                text=f"Normalized to file {global_normalization_point}"
            )
            ########################################################################
    ...
    def reset_ratiometric_data(self) -> None:
        ...
        for file in global_file_list:
            ...
            for num in range(global_electrode_count):
                ...
                global_normalized_ratiometric_data_list[num][index] = (
                    normalized_data_ratio
                )
                # -- KDM ---#
                match global_kdm_method:
                    case KDMMethod.OLD:
...
class PostAnalysis(ttk.Frame):
    ...
    def _initialize_frame(self) -> None:
        ...
        ttk.Label(self, text="Post Analysis", font=HUGE_FONT).grid(
            row=0, column=0, columnspan=2
        )
        data_adjustment_frame = ttk.Frame(self, relief="groove")
        data_adjustment_frame.grid(
        ...
        # --- Real-time Normalization Variable ---#
        ttk.Label(
            normalization_frame, text="Set Normalization Point", font=MEDIUM_FONT
        ).grid(row=0, column=0, pady=5)
        normalization_var = tk.StringVar()
        norm_string = str(global_normalization_point)
        normalization_var.set(norm_string)
        self.set_point_norm = ttk.Entry(
            normalization_frame, textvariable=normalization_var, width=8
        )
        ...
        ttk.Button(
            normalization_frame,
            text="Apply Norm",
            command=self.post_analysis_normalization,
            width=10,
        ).grid(row=2, column=0)
        self.norm_warning = ttk.Label(
            normalization_frame, text="", foreground="red", font=MEDIUM_FONT
        )
        if len(global_frequency_list) > 1:
            ...
            # --- Drift Correction Title ---#
            ttk.Label(
                self.frequency_frame, text="Drift Correction", font=LARGE_FONT
            ).grid(row=0, column=0, columnspan=3, pady=1, padx=5)
            # --- High Frequency Selection for KDM and Ratiometric Analysis ---#
            ttk.Label(
                self.frequency_frame, text="High Frequency", font=MEDIUM_FONT
        ...
        ttk.Button(
            data_adjustment_frame, text="Redraw Figures", command=self._draw, width=12
        ...
        row_value += 1
        # --- Reset ---#
        ttk.Button(self, text="Reset", style="Fun.TButton", command=self._reset).grid(
            row=row_value, column=1, pady=5, padx=5
        )
        # --- Quit ---#
        ttk.Button(self, text="Quit", command=lambda: os._exit(0)).grid(
    ...
    def _draw(self) -> None:
        ...
        for num in range(global_electrode_count):
            ...
            for count in range(len(global_frequency_list)):
                ...
                axs[1][count].clear()
                axs[2][count].clear()
                supposedly_global_norm = axs[2][count].plot(
                    x_axis, local_normalized_data_list, "ko", markersize=1
                )
                #####################
                ## Set the Y Label ##
                #####################
            ...
            if len(global_frequency_list) > 1:
                ...
                supposedly_global_norm = [
                    x * 100 for x in global_normalized_ratiometric_data_list[num]
                ]
    ...
    def post_analysis_kdm(self) -> None:
        ...
        # --- Reset the variable for the Warning Label (WrongFrequencyLabel) ---#
        if (
            global_high_frequency not in global_frequency_list
            ...
            global_high_frequency in global_frequency_list
    ...
    def data_export_top_level(self) -> None:
        ...
        self.no_selected_path = ttk.Label(
            self.win, text="No File Path Selected", font=MEDIUM_FONT, foreground="red"
        )
        self.path_warning_exists = False
        # --- File Handle Input ---#
        ttk.Label(self.win, text="Exported File Handle:", font=LARGE_FONT).grid(
        ...
        for frequency in global_frequency_list:
            self.frequency_list.insert(tk.END, frequency)
        ttk.Button(
            self.win, text="Export Data", command=self.post_analysis_data_export
        ).grid(row=15, column=0, columnspan=2)
        ttk.Button(self.win, text="Close", command=self.win.destroy).grid(
            row=16, column=0, columnspan=2, pady=10
        )
            ...
            case AnalysisMethod.CONTINUOUS_SCAN:
                ...
                match global_plot_summary_mode:
                    case PlotSummaryMode.PHE:
                        ...
                        with open(
                            self.text_file_handle, "w+", encoding="utf-8", newline=""
                        ) as output:
                ...
                for frequency in self.frequency_list:
                    for electrode in self.electrode_list:
                        match global_plot_summary_mode:
                            case PlotSummaryMode.PHE:
                                txt_list.append(
                                    "PeakHeight_E%d_%dHz" % (electrode, frequency)
                                )
                                match global_peak_method:
                                    case PeakMethod.GAUSS:
                                        txt_list.append(
                                            "PeakLocation_E%d_%dHz"
                                            % (electrode, frequency)
                                        )
                            case PlotSummaryMode.AUC:
                ...
                with open(
                    self.text_file_handle, "a", encoding="utf-8", newline=""
                ) as output:
                    writer = csv.writer(output, delimiter=" ")
                    writer.writerow(txt_list)
            case AnalysisMethod.FREQUENCY_MAP:
    ...
    def continuous_scan_export(self, file: int) -> None:
        ...
        for count in range(len(global_frequency_list)):
            for num in range(global_electrode_count):
                if (
                    ...
                    output_list.append(
                        str(global_offset_normalized_data_list[num][index])
                    )
                else:
                    output_list.append(
                        str(global_normalized_data_list[num][count][index])
                    )
            ...
            for count in range(len(global_frequency_list)):
                ...
                for num in range(global_electrode_count):
                    if (
                        ...
                        normalized_frequency_currents.append(
                            global_offset_normalized_data_list[num][index]
                        )
                    else:
                        normalized_frequency_currents.append(
                ...
                average_norm = (
                    sum(normalized_frequency_currents) / global_electrode_count
                )
        ...
        if self.electrode_count > 1:
            for count in range(len(global_frequency_list)):
                ...
                std_list = [
                    (x - average_norm) ** 2 for x in normalized_frequency_currents
                ]
                standard_deviation = float(
                    sqrt(sum(std_list) / (global_electrode_count - 1))
                )
        ...
        if len(global_frequency_list) > 1:
            ...
            for num in range(global_electrode_count):
                output_list.append(
                    str(global_normalized_ratiometric_data_list[num][index])
                )
            ...
            if self.electrode_count > 1:
                ...
                norm_std_list: List[float] = [
                    (x - norm_average) ** 2 for x in norm_list
                ]
                norm_standard_deviation = sqrt(
                    sum(norm_std_list) / (global_electrode_count - 1)
                )
        ...
        with open(
            self.text_file_handle, "r", encoding="utf-8", newline=""
        ) as filecontents:
        ...
        try:
            ...
            if self.electrode_count > 1:
                ...
                charge_standard_deviation = sqrt(
                    sum(std_list) / (global_electrode_count - 1)
                )
            ...
            for index in range(analysis_range):
                ...
                for frequency in self.frequency_list:
                    ...
                    for electrode in self.electrode_list:
                        ...
                        match global_peak_method:
                            case PeakMethod.GAUSS:
                                txt_list.append(
                                    str(global_peak_list[num][count][index])
                                )
                # --- Avg. Peak Height ---#
                if self.electrode_count > 1:
                    for frequency in self.frequency_list:
                ...
                if self.electrode_count > 1:
                    for frequency in self.frequency_list:
                        ...
                        average_norm = (
                            sum(normalized_frequency_currents) / self.electrode_count
                        )
                    ...
                    for frequency in self.frequency_list:
                        ...
                        std_list = [
                            (x - average_norm) ** 2
                            for x in normalized_frequency_currents
                        ]
                        standard_deviation = sqrt(
                            sum(std_list) / (self.electrode_count - 1)
                        )
                ...
                if len(self.frequency_list) > 1:
                    ...
                    if self.electrode_count > 1:
                        ...
                        norm_standard_deviation = sqrt(
                            sum(norm_std_list) / (self.electrode_count - 1)
                        )
                    ...
                    if self.electrode_count > 1:
                        ...
                        kdm_std_list: List[float] = [
                            (x - kdm_average) ** 2 for x in kdm_list
                        ]
                        kdm_std: float = sqrt(
                            sum(kdm_std_list) / (self.electrode_count - 1)
                        )
...
global_frame_reference: Union[
    ContinuousScanVisualizationFrame, FrequencyMapVisualizationFrame
]
...
global_wrong_frequency_label: ttk.Label
global_plot_list_continuous_scan: List[
    List[
        Tuple[
            matplotlib.lines.Line2D,
            matplotlib.lines.Line2D,
...
global_plot_list_frequency_map: List[
...
global_ratiometric_figures: List[
    Tuple[matplotlib.figure.Figure, List[matplotlib.axes.Axes]]
]
############################################
...
if __name__ == "__main__":
    ...
    style = ttk.Style()
    style.configure(
        "On.TButton", foreground="blue", font=MEDIUM_FONT, relief="raised", border=100
    )
    style.configure(
